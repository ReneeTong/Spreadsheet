...
namespace SS
{
    ...
    public abstract class AbstractSpreadsheet
    {
        ...
        /// this method a cell's all dependent cell and check if they are
        /// rely on themself, which is, if there exist an circularException.
        ///
        /// if not, it will add all the cells that's directly or indircly relates to the passing cells
        ///
        /// For example, if we have cell a1 contain (b1 + c1), and cell b1 contain(c1+2), cell c1 contain(a1)
        /// it will first mark a1 as visited, and check a1's dependents, which is c1
        /// it will then mark c1 as visited, and check c1's dependents, which is a1 and b1.
        /// if it goes to a1, it will find out a1 is visited therefore a circular dependents exist
        /// if it goes to b1, it will check it's dependents a1, and find out a1 is already visited, and circular dependents exist
        ///
        /// If there's no circular dependents, let's say a1 contain (c1+2), b1 contain (a1+3), c1 contain (3)
        /// it will go to a1, mark it as visited, and go to its dependents b1
        /// it will mark b1 as visited, and return a hashset that contain {a1, b1} which is all the cells that's directly and indirctly depends on a1
        /// </summary>
